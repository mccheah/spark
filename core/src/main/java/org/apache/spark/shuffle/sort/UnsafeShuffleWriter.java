...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  void closeAndWriteOutput() throws IOException {
    ...
    sorter = null;
    final ShuffleMapOutputWriter writer = shuffleExecutorComponents
        .writes()
        .createMapOutputWriter(shuffleId, mapId, partitioner.numPartitions());
    final long[] partitionLengths;
    try {
      try {
        partitionLengths = mergeSpills(spills, writer);
      } finally {
      ...
      try {
        ...
        logger.error("Failed to abort map output writer after failing the write.", e2);
      }
      throw e;
    }
   ...
   */
  private long[] mergeSpills(SpillInfo[] spills, ShuffleMapOutputWriter writer)
      throws IOException {
    ...
    if (spills.length == 0) {
      ...
      try (FileInputStream spillFileInput = new FileInputStream(spills[0].file)) {
        for (int partition = 0; partition < spills[0].partitionLengths.length; partition++) {
          try (ShuffleBlockOutputStream blockOutput = writer.getNextPartitionOutputStream()) {
            for (int i = 0; i < spills[0].partitionLengths[partition]; i++) {
              blockOutput.write(spillFileInput.read());
            }
          }
        }
      }
      ...
      if (fastMergeEnabled && fastMergeIsSupported) {
        ...
        if (transferToEnabled
          ...
          partitionLengths = mergeSpillsWithTransferTo(
              spills, (ShuffleMapOutputChannelWriter) writer);
        } else {
   ...
   * the NIO-based merge,
   * {@link UnsafeShuffleWriter#mergeSpillsWithTransferTo(
   * SpillInfo[], ShuffleMapOutputChannelWriter)}],
  ...
  private long[] mergeSpillsWithFileStream(
    ...
    try {
      ...
      for (int partition = 0; partition < numPartitions; partition++) {
        ShuffleBlockOutputStream backendPartitionOutput = null;
        ...
        try {
          backendPartitionOutput = writer.getNextPartitionOutputStream();
          partitionOutput = new TimeTrackingOutputStream(writeMetrics, backendPartitionOutput);
          ...
          if (backendPartitionOutput != null) {
            backendPartitionOutput.close();
          }
        }
        if (backendPartitionOutput != null) {
          partitionLengths[partition] = backendPartitionOutput.getCount();
          writeMetrics.incBytesWritten(backendPartitionOutput.getCount());
        }
      }
  ...
  private long[] mergeSpillsWithTransferTo(
      SpillInfo[] spills, ShuffleMapOutputChannelWriter writer) throws IOException {
    assert (spills.length >= 2);
    final int numPartitions = partitioner.numPartitions();
    final long[] partitionLengths = new long[numPartitions];
    ...
    try {
      ...
      for (int partition = 0; partition < numPartitions; partition++) {
        ShuffleBlockByteChannel outputChannel = null;
        try {
          outputChannel = writer.getNextPartitionByteChannel();
          for (int i = 0; i < spills.length; i++) {
            final long partitionLengthInSpill = spills[i].partitionLengths[partition];
        ...
        if (outputChannel != null) {
          writeMetrics.incBytesWritten(outputChannel.getCount());
        }
      }
      threwException = false;
    } finally {
