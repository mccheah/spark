...
public class DefaultShuffleExecutorComponents implements ShuffleExecutorComponents {
  ...
  public void intitializeExecutor(String appId, String execId) {
    blockManagerId = SparkEnv.get().blockManager().blockManagerId();
    blockResolver = new IndexShuffleBlockResolver(sparkConf, SparkEnv.get().blockManager());
  }
  ...
  public ShuffleWriteSupport writes() {
    if (blockManagerId == null || blockResolver == null) {
      throw new IllegalStateException(
          "Executor components must be initialized before getting writers.");
    }
    return new DefaultShuffleWriteSupport(sparkConf, blockManagerId, blockResolver);
  }
}
